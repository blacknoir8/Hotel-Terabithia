...
let auditorioColoradoCapacity = 300;
let auditorioLaranjaChairs = 80;
...
function reservadequarto() {
    ...
    let confirmation = prompt(
        `${userName}, você confirma a hospedagem para ${guestName} por ${stayDuration} dias? (S/N)`);

    if (confirmation.toUpperCase() === "S") {
        let totalCostForStay = calculateCost(stayDuration, hotelRate);
        alert(
            `${userName}, reserva efetuada para ${guestName}. O valor total é de R$${totalCostForStay.toFixed(2)}.`);
...
function checkGratisOrHalfPrice(name, duration) {
    ...
    if (nextGuest.toUpperCase() !== "PARE") {
        ...
        alert(
            `${userName}, o valor total das hospedagens é: R$${totalCost.toFixed(2)}; ${totalGratis} gratuidade(s); ${totalHalfPrice} meia(s)`);
    }
}
...
function buffet() {
    ...
    alert(
        `O evento precisará de ${cafeLiters} litros de café, ${aguaLiters} litros de água, ${salgados} salgados. O custo total do evento será de R$ ${totalBuffetCost.toFixed(2)}`);
...
function orçamento() {
    ...
    do {
        ...
        let totalServiceCost = calculateServiceCost(serviceRate, serviceQuantity, serviceDiscount,
            serviceMinQuantity);
...
function cadastrodehospede() {
    while (true) {
        var hospedagemcadastrar = prompt(
            "Selecione uma opção: \n1. Cadastrar  \n2. Pesquisar  \n3. Listar  \n4. Voltar");
